...
// Preface and table-of-contents are generated by `thesis.with(...)` above.
// (Removed explicit top-level macro calls to avoid duplicate literal output.)
...
#algorithm(
  caption: [Adaptive feature selection algorithm],
  title: [Adaptive Feature Selection],
  [
    + *Input:* Dataset $cal(D)$, initial feature set $F_0$, threshold $tau$
    + *Output:* Selected feature subset $F^*$
    + Initialize $F <- F_0$, $w <- 0^d$
    + *while* not converged *do*
      + Train model on current features $F$
      + Compute feature importance scores $s_i$ for $i in F$
...
// Ensure appendix B starts on a fresh page and its content is breakable to avoid
